<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    ...
    <?php if ( has_post_thumbnail() && option::is_on('display_thumb') ) { ?>
    ...
    <section class="entry-body">
        ...
        <?php if (option::get('post_view') == 'Grid' ) { ?><?php if ( option::is_on( 'display_category' ) ) printf( '<span class="cat-links">%s</span>', get_the_category_list( ' ' ) ); ?><?php } ?>

        <?php the_title( sprintf( '<h3 class="entry-title"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h3>' ); ?>

        <div class="entry-meta">
           <?php if (option::get('post_view') == 'List' ) { ?><?php if ( option::is_on( 'display_category' ) ) printf( '<span class="cat-links">%s</span>', get_the_category_list( ' ' ) ); ?><?php } ?>
